...
(*)构造方法调用就报错(如Navigator、Location、HTMLDivElement)，但可以通过另外一个方法创建(如document.createElement可以，而HTMLDivElement就报错）
    解决：
        1)、自己写一个方法，创建的实例，其原型对象(__proto__)指向构造方法的原型对象(prototype)。
                1、参照bom系列的对象(如location, navigator)； 
                2、参照document.createElement("div")中创建一个新对象，该新对象的原型为HTMLDivElement.prototype, 这样该新对象 instanceof HTMLDivElement的结果就为true了。
...
【*】
    ...
    *)proxy代理的对象有控制开关。
    *)代码中的debugger不用管，正常模式不会暂停住，就算是调试模式也可以鼠标右键让程序不会在此处暂停。
